...
class App:
    ...
    def generate_rollback_task_definition(self, latest_task_definition):
        ...
        if rollback_task_definition_version <= 0:
            raise Exception('Rollback is not possible as the task definition version is invalid')
        task_definition_prefix = ":".join(latest_task_definition.split(':')[:-1])
        rollback_task_definition = f"{task_definition_prefix}:{rollback_task_definition_version}"
    ...
    def rollback(self):
        ...
        rollback_task_definition = self.generate_rollback_task_definition(latest_task_definition) 
